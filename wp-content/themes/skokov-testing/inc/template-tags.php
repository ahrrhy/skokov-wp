...
function skokov_testing_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'skokov-testing' ),
...
function skokov_testing_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'skokov-testing' ) );
		if ( $categories_list && skokov_testing_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'skokov-testing' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'skokov-testing' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'skokov-testing' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', 'skokov-testing' ),
